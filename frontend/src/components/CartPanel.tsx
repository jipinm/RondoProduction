...
/** Collapsible hospitality details shown below each cart item */
const CartHospitalityDetails: React.FC<{ hospitalities: { hospitality_id: number; name: string; description?: string | null }[] }> = ({ hospitalities }) => {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className={styles.hospitalitySection}>
      <button
        className={styles.hospitalityToggle}
        onClick={() => setExpanded(!expanded)}
        type="button"
      >
        <ChefHat size={12} />
        <span>{hospitalities.map(h => h.name).join(', ')}</span>
        ...
        <div className={styles.hospitalityList}>
          {hospitalities.map(h => (
            <div key={h.hospitality_id} className={styles.hospitalityItem}>
              <div className={styles.hospitalityItemContent}>
                <span className={styles.hospitalityName}>{h.name}</span>
                {h.description && (
                  <div
                    className={styles.hospitalityDescription}
              ...
              <div className={styles.cartItems}>
                {cartItems.map(item => (
                  <div key={item.ticket.ticket_id} className={styles.cartItem}>
                    ...
                    {/* Hospitality details — full width below the ticket row */}
                    {item.includedHospitalities && item.includedHospitalities.length > 0 && (
                      <CartHospitalityDetails hospitalities={item.includedHospitalities} />
                    )}
                  </div>
                ))}
