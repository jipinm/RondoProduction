...
interface UseGolfTournamentsReturn {
  tournaments: Tournament[];
  loading: boolean;
  error: string | null;
}

export const useGolfTournaments = (): UseGolfTournamentsReturn => {
  const [tournaments, setTournaments] = useState<Tournament[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const fetchTournaments = useCallback(async () => {
    const cacheKey = apiCache.getGolfTournamentsKey();
    ...
    try {
      ...
      const tournamentsUrl = `${API_ENDPOINTS.TOURNAMENTS}?page_size=50&page=1&sport_type=golf&date_stop=ge:${today}`;
      ...
      // Cache for 20 minutes – same duration as other sport menu hierarchies
      apiCache.set(cacheKey, filteredTournaments, 20);
      setTournaments(filteredTournaments);
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to fetch golf tournaments';
